...
std::vector<std::unique_ptr<Room>> ReadDungeon(std::istream& in) {
    ...
    for (auto i = 0ul; i < dungeon.size(); ++i) {
...
TEST_CASE("GeneratedDungeons") {
    ...
    const auto num_tests = 10;
    const auto num_rooms = 100;
    const auto num_doors = 200;
    const auto num_closed_doors = 100;

    for (auto k = 0; k < num_tests; ++k) {
        std::vector<std::unique_ptr<Room>> dungeon(num_rooms);
        for (auto i = 0ul; i < dungeon.size(); ++i) {
            dungeon[i].reset(new Room(i + 1 == dungeon.size()));
        }
        ...
        for (auto i = 0; i < num_doors; ++i) {
            auto from = rnd.GenInt(0, num_rooms - 1);
            auto to = rnd.GenInt(0, num_rooms - 1);
        ...
        for (auto i = 0; i < num_closed_doors; ++i) {
            auto from = rnd.GenInt(0, num_rooms - 1);
            auto to = rnd.GenInt(0, num_rooms - 1);
            auto key_location = rnd.GenInt(0, num_rooms - 1);
        ...
        auto* final_room = FindFinalRoom(dungeon.front().get());
